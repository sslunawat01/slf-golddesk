...
eq("scope defaults to every branch (mig 019: scopeAll)",
  validSlfBank(base).scopeAll, true);
eq("scopeAll false with no ticks = deliberately parked (owner №3)",
  JSON.stringify([validSlfBank({ ...base, scopeAll: false }).scopeAll,
                  validSlfBank({ ...base, scopeAll: false }).branchIds]), "[false,[]]");
eq("ticked branches come back as a clean id list",
  JSON.stringify(validSlfBank({ ...base, scopeAll: false, branchIds: [2, "1", 2] }).branchIds), "[2,1]");
